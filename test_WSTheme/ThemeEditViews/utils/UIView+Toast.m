...
- (CGPoint)centerPointForPosition:(ToastPosition)position withToast:(UIView *)toast;
...
- (void)makeToast:(NSString *)message {
    [self makeToast:message duration:CSToastDefaultDuration position:ToastPositionBottom];
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(ToastPosition)position title:(NSString *)title {
    UIView *toast = [self viewForMessage:message title:title image:nil];
    [self showToast:toast duration:duration position:position];  
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(ToastPosition)position image:(UIImage *)image {
    UIView *toast = [self viewForMessage:message title:nil image:image];
    [self showToast:toast duration:duration position:position];  
}

- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(ToastPosition)position title:(NSString *)title image:(UIImage *)image {
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration  position:(ToastPosition)position title:(NSString *)title image:(UIImage *)image tapCallback:(void(^)(void))tapCallback{
  UIView *toast = [self viewForMessage:message title:title image:image];
  [self showToast:toast duration:duration position:position tapCallback:tapCallback];
}

- (void)showToast:(UIView *)toast {
    [self showToast:toast duration:CSToastDefaultDuration position:ToastPositionBottom];
}


- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(ToastPosition)position {
    [self showToast:toast duration:duration position:position tapCallback:nil];
    
}


- (void)showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(ToastPosition)position tapCallback:(void(^)(void))tapCallback
{
    if (mShowView) {
...
- (void)makeToastActivity {
    [self makeToastActivity:ToastPositionCenter];
}
...
- (CGPoint)centerPointForPosition:(ToastPosition)point withToast:(UIView *)toast {
  ...
  switch (point) {
    ...
    {
        int topLayoutLength = 0;
        if (myVC) {
            topLayoutLength = myVC.topLayoutGuide.length; // ios7 - 11
        }
      return CGPointMake(self.bounds.size.width/2, (toast.frame.size.height / 2) + CSToastVerticalPadding+topLayoutLength);
    } break;
    ...
    default: // default to bottom
      {
          int bottomLayoutLength = 0;
          if (myVC) {
              bottomLayoutLength = myVC.bottomLayoutGuide.length;
          }
          return CGPointMake(self.bounds.size.width/2, (self.bounds.size.height - (toast.frame.size.height / 2)) - CSToastVerticalPadding - bottomLayoutLength);
      }
      break;
  }
}
