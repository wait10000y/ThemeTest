...
- (void)showImagePickerControllerWithSourceType:(UIImagePickerControllerSourceType)sourceType presentingViewController:(UIViewController *)presentingViewController completionHandler:(SSImagePickerHelperDidCaptureImageBlock)completionHandler {
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) {
        imagePickerController.popoverPresentationController.sourceView = presentingViewController.view;
        CGRect tempRect = presentingViewController.view.bounds;
        imagePickerController.popoverPresentationController.sourceRect = CGRectMake(CGRectGetWidth(tempRect), 2, 1, 1);
    }
    //解注释这条语句，在拍照界面可以选择视频
